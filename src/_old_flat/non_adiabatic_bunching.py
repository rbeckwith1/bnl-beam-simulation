"""
AGS non-adiabatic bunch-compression simulation.

This is the non-adiabatic (fast RF voltage jump) counterpart to the
resonant-bunching script, rebuilt on the same infrastructure so the two
studies are directly comparable:

- Time: nanoseconds internally (t, T0_ns, T_rf_ns), consistent with the
  Vrf * sin(phase) kick being in GeV per turn.
- All "expensive" one-time calculations (synchrotron frequency by two
  independent methods, matched-ellipse amplitudes from a target
  emittance, separatrix machinery setup) are done ONCE, before the main
  per-turn tracking loop.
...
- The driving term is a non-adiabatic voltage PROGRAM (hold low, then a
  fast smoothstep jump to high voltage) rather than a resonant modulation.
"""
...
# Non-interactive backend; we render straight to mp4, no display needed.
matplotlib.use("Agg")
...
# =====================================================================
parser = argparse.ArgumentParser(description="AGS non-adiabatic bunching simulation")
...
OUTPUT_CSV = "non_adiabatic_bunching_diagnostics.csv"
OUTPUT_MP4 = "non_adiabatic_bunching_animation.mp4"
...
# ---------------------------------------------------------------------
# Non-adiabatic RF voltage program: hold low, then a fast smoothstep
# jump to high voltage.
# ---------------------------------------------------------------------
Vrf_low = 20e3 / 1e9         # 20 kV -> GeV (pre-jump voltage)
Vrf_high = 320e3 / 1e9       # 320 kV -> GeV (post-jump voltage)
jump_start_turn = 2000         # turn at which the jump begins
jump_turns = 50               # turns over which the jump completes
...
# once a_coef/b_coef (the linear ellipse aspect ratio, evaluated at the
# PRE-JUMP voltage Vrf_low, since that's the matched state the bunch starts
# in) are known, from the requirement that the matched ellipse encloses
# exactly eps_l_ns_GeV.
...
# Stop shortly after the strongest compression (post-jump), rather than
# running the full n_turns once the bunch starts re-lengthening.
STOP_AFTER_BEST_COMPRESSION = True
ROWS_PAST_BEST_TO_STOP = 20
...
# Linearize about the PRE-JUMP voltage, since that's the matched state the
# bunch is initialized in (before the non-adiabatic jump perturbs it).
...
J_exact = compute_exact_jacobian(Vrf_low)
...
print()
print("=" * 70)
print("SYNCHROTRON FREQUENCY / PERIOD -- METHOD COMPARISON (pre-jump voltage)")
print("=" * 70)
...
# Use the exact (Jacobian) estimate as the working omega_s, since it
# captures the exact drift map (Method 1 is used only as an independent
# cross-check and for the matched-ellipse amplitude relation below).
omega_s = omega_s_jac
T_s_turns = T_s_jac_turns

print(f"Non-adiabatic jump reference: T_s(pre-jump) = {T_s_turns:.1f} turns, "
      f"vs. jump_turns = {jump_turns} turns "
      f"({jump_turns / T_s_turns:.3f} x T_s -- well under 1 confirms the "
      f"jump is genuinely non-adiabatic, i.e. fast compared to synchrotron "
      f"motion).")
print()

# =====================================================================
# NON-ADIABATIC RF VOLTAGE PROGRAM
# =====================================================================
def voltage_program(turn):
    """
    RF voltage [GeV] at the given turn: holds Vrf_low, then transitions to
    Vrf_high over jump_turns using a smoothstep (3r^2 - 2r^3) profile, which
    has zero slope at both ends (no kink in dV/dn at the jump boundaries)
    but is still fast/non-adiabatic relative to T_s (see printout above).
    """
...
# machine at the pre-jump voltage (independent of ellipse size), using the
# LINEAR (a,b) coefficients -- this is itself a linear-theory result used
# only to *initialize* the bunch shape matched to the pre-jump bucket;
# subsequent tracking is fully nonlinear.
#
...
print(f"Matched-ellipse initial amplitudes (pre-jump bucket): "
      f"a_t = {a_t:.3f} ns, a_E = {a_E*1e3:.4f} MeV")
...
r_init = np.sqrt(np.random.rand(N))   # sqrt gives uniform filling of the ellipse
...
# largest voltage used anywhere in this script (Vrf_high) is estimated via
# a coarse search and padded by a comfortable safety factor.
def _estimate_bucket_dE_scale():
    from scipy.integrate import quad
    Vrf_hi = Vrf_high * 1.2   # safety margin
...
for n in range(n_turns):
    ...
    if n % LOG_EVERY == 0:
        ...
        # Quadrupole (envelope) moments, normalized to the initial matched
        # ellipse -- useful here too, since a fast non-adiabatic jump can
        # excite the same 2*omega_s envelope oscillation that the resonant
        # study drives on purpose.
        ...
        # --- early stop shortly after strongest compression -------------
        if STOP_AFTER_BEST_COMPRESSION and n > jump_start_turn + jump_turns:
            if time_sigma < best_time_sigma:
                best_time_sigma = time_sigma
                best_turn = n
                rows_since_best = 0
            else:
                rows_since_best += 1
...
if stop_turn is not None:
    print(f"Stopped early at turn {stop_turn}: strongest compression was at "
          f"turn {best_turn} (time_sigma = {best_time_sigma:.4f} ns), "
          f"{ROWS_PAST_BEST_TO_STOP} logged rows past that with no further "
          f"improvement.")
...
if MAKE_ANIMATION and len(snapshot_turns) > 0:
    ...
    ax.set_xlim(-t_plot_lim/5, t_plot_lim/5)
    ...
    def update_anim(frame_idx):
        ...
        info_text.set_text(
            ...
            f"T_s (pre-jump, est.) = {T_s_turns:.1f} turns\n"
            f"jump: start={jump_start_turn}, dur={jump_turns} turns"
        )
        return scat, sep_pos_line, sep_neg_line, info_text
...
save_plot(df.turn, df.Vrf_kV, "Turn", "V_rf [kV]",
          "RF voltage vs. turn (non-adiabatic jump)", "plot_1_Vrf_vs_turn.png")
...
ax.set_title("Q1, Q2 vs. turn (envelope response to the jump)")
ax.legend()
ax.grid(alpha=0.3)
fig.tight_layout()
...
print(f"Minimum RMS bunch length reached: {df.time_sigma_ns.min():.3f} ns "
