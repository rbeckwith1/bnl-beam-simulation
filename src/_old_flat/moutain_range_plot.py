...
# ---------------------------------------------------------------------
TARGET_INITIAL_EPS_EVS = 1.09   # pre-squeeze proxy from the note (40 ms
...
# ---------------------------------------------------------------------
# Mountain-range plot: separate, coarser snapshot cadence.
# The animation snapshot cadence above is tuned for smooth mp4 playback
# (hundreds of frames); a static waterfall plot wants far fewer traces
# (real BNL WCM mountain-range figures typically show ~80) or it turns
# into an illegible smear. Collected independently so changing one
# cadence doesn't affect the other.
# ---------------------------------------------------------------------
MOUNTAIN_RANGE_TARGET_TRACES = 80 if not DEBUG_MODE else 30
mr_save_snapshot_every = max(1, n_turns // MOUNTAIN_RANGE_TARGET_TRACES)
mr_snapshot_times = []
mr_snapshot_turns = []
...
print(f"  mountain-range snapshot cadence: every {mr_save_snapshot_every} turns "
      f"(~{n_turns // mr_save_snapshot_every} traces)")
print("=" * 70)
...
for n in range(n_turns):
    ...
    if n % mr_save_snapshot_every == 0:
        mr_snapshot_times.append(time.copy())
        mr_snapshot_turns.append(n)

print(f"Tracking complete. Logged {len(diagnostics_rows)} diagnostic rows, "
      f"{len(snapshot_turns)} animation snapshots, "
      f"{len(mr_snapshot_turns)} mountain-range snapshots.")
print()
...
# =====================================================================
# MOUNTAIN-RANGE (WCM-style waterfall) PLOT
# =====================================================================
# Real wall-current-monitor mountain-range figures (e.g. BNL-223784-2022-TECH,
# Figs. 2/13/14/15/19) stack many single-turn line-density traces, each
# offset upward, with later traces on top and a phosphor-scope look (dark
# background, single bright trace color, thin lines, no legend). We build
# this from mr_snapshot_times by histogramming each snapshot's `time` array
# (a proxy for "charge vs. time-within-bucket" at that instant) and drawing
# each histogram as one offset curve.
def make_mountain_range_plot(snapshot_times_list, snapshot_turns_list,
                              fname="plot_6_mountain_range.png",
                              n_bins=200, half_range_ns=None):
    if len(snapshot_times_list) == 0:
        print("Mountain-range plot skipped (no snapshots).")
        return

    if half_range_ns is None:
        # Match the animation's horizontal window so the bunch shape is
        # resolved without excess empty margin.
        half_range_ns = 8.0 * a_t

    bin_edges = np.linspace(-half_range_ns, half_range_ns, n_bins + 1)
    bin_centers = 0.5 * (bin_edges[:-1] + bin_edges[1:])

    profiles = []
    for t_snap in snapshot_times_list:
        counts, _ = np.histogram(t_snap, bins=bin_edges)
        profiles.append(counts.astype(float))
    profiles = np.array(profiles)  # shape (n_snapshots, n_bins)

    # Normalize all profiles by the single global peak so relative
    # amplitude changes (bunch compressing/growing) stay visible across
    # the stack, rather than each trace being independently re-normalized.
    global_peak = profiles.max() if profiles.max() > 0 else 1.0
    profiles_norm = profiles / global_peak

    n_snap = len(snapshot_times_list)
    # Vertical spacing: a fraction of the normalized peak height so
    # neighboring traces overlap a bit (as in the real instrument plots)
    # without fully obscuring each other.
    offset_step = 0.55
    ...
    trace_color = "#39FF14"  # phosphor green

    # index 0 = earliest turn -> plotted at the bottom (offset 0);
    # later turns get larger offsets -> appear higher, per "later on top".
    for idx in range(n_snap):
        offset = idx * offset_step
        ax.plot(bin_centers, profiles_norm[idx] + offset,
                color=trace_color, lw=0.7, alpha=0.9,
                solid_capstyle="round")

    ax.set_xlim(-half_range_ns, half_range_ns)
    ax.set_ylim(-0.1, (n_snap - 1) * offset_step + 1.3)

    ax.set_xlabel("Time within bucket [ns]", color="white")
    ax.set_yticks([])
    ax.set_title(
        f"Simulated WCM mountain-range plot  "
        f"(turns {snapshot_turns_list[0]}-{snapshot_turns_list[-1]}, "
        f"{n_snap} traces, later on top)",
    ...
    plt.close(fig)
    print(f"  saved {fname}  ({n_snap} traces)")


print()
print("Generating mountain-range plot...")
make_mountain_range_plot(mr_snapshot_times, mr_snapshot_turns)
