"""
Resonant (parametric) longitudinal bunching simulation for the BNL AGS
========================================================================

Physical picture
-----------------
A proton bunch circulating in the AGS above transition is tracked with an
exact one-turn drift-and-kick map:

    (1) DRIFT : the particle's arrival-time deviation from the synchronous
        particle changes because its energy deviation changes its momentum,
        velocity, and (through momentum compaction) its orbit length.
    (2) KICK  : the RF cavity changes the particle's energy deviation
        according to its RF phase, using the exact sinusoidal RF voltage
        (not the linearized approximation).

A matched bunch (uniformly filled phase-space ellipse) has an envelope
(second-moment) shape that is stationary under a *linear* map, but under a
*periodically modulated* RF voltage, if the modulation frequency omega_mod is
close to 2*omega_s (omega_s = single-particle small-amplitude synchrotron
frequency), the envelope's quadrupole (breathing/shearing) degree of freedom
can be parametrically driven -- this is the classic Mathieu/Hill-type
parametric resonance. The physical justification for the 2*omega_s condition
and other conceptual points are summarized in the accompanying chat message.

This script studies whether that resonance can be used to controllably
compress the AGS bunch length toward ~2 ns RMS.

Coding conventions
-------------------
- Energies: GeV internally (dE, Vrf, F(dE), etc.), converted to MeV only for
  logging / plotting.
...
a_t = 40.33/2                 # ns, initial matched-ellipse time half-width with AGS parameter from 2024 run
...
# =====================================================================
# a_E = a_t * sqrt(|b/a|)   [GeV], using the LINEAR (a,b) coefficients,
# since this matched-ellipse relation is itself a linear-theory result used
# only to *initialize* the bunch shape; subsequent tracking is fully
# nonlinear.
a_E = a_t * np.sqrt(abs(b_coef / a_coef))     # GeV
